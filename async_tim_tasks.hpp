    ...
    * @brief to use:
      ...
      to place a task:
          type macro PLACE_ASYNC_TASK(
                1st @arg -> lambda [](void* self_ptr_as_context){static_cast<SelfT*>(context)->...}
          ...
          to put max frequently task use PLACE_ASYNC_QUICKEST(
                @arg -> lambda [](void* self_ptr_as_context){static_cast<SelfT*>(context)->...}
            )
            @retval task N in pool(std::size_t)
...
#define PLACE_ASYNC_SUSPENDED_QUICKEST(lambda)  \
                    async_tim_task_impl::TaskPool::GetPool().PlaceToPool(task::CB(this, lambda), true)
